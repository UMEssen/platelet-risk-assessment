...
class SharpnessAwareMinimizationPatched(Model):
    ...
    def train_step(self, data):
        ...
        if self.num_batch_splits is not None:
            x_split_1 = tf.split(x[0], self.num_batch_splits)
            x_split_2 = tf.split(x[1], self.num_batch_splits)
            x_split_3 = tf.split(x[2], self.num_batch_splits)
            x_split = [
                (x_split_1[count], x_split_2[count], x_split_3[count])
                for count in range(self.num_batch_splits)
            ]
        ...
        for gradient_all_batches in gradients_all_batches:
            gradients.append(
                tf.reduce_sum(
                    [tf.convert_to_tensor(x) for x in gradient_all_batches], axis=0
                )
            )
